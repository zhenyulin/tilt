...
func wireDemo(ctx context.Context, branch demo.RepoBranch) (demo.Script, error) {
	...
	execCustomBuilder := build.NewExecCustomBuilder(cli, clock)
	engineUpdateModeFlag := provideUpdateModeFlag()
	updateMode, err := engine.ProvideUpdateMode(engineUpdateModeFlag, env)
	if err != nil {
		return demo.Script{}, err
	}
	imageBuildAndDeployer := engine.NewImageBuildAndDeployer(imageBuilder, cacheBuilder, execCustomBuilder, client, env, analytics, updateMode, clock)
